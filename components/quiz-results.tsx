...
import { Check, X, Trophy, Clock, Target, RotateCcw, Download } from 'lucide-react';
...
export function QuizResults({ quiz, session, onRetry, onHome }: QuizResultsProps) {
  ...
  const correctCount = quiz.questions.filter(q => checkAnswer(session.answers[q.id], q.correctAnswer)).length;
  const answeredCount = Object.keys(session.answers).length;
  const totalPoints = quiz.questions.reduce((sum, q) => sum + (q.points || 1), 0);
  const earnedPoints = quiz.questions.reduce((sum, q) => {
    return checkAnswer(session.answers[q.id], q.correctAnswer) ? sum + (q.points || 1) : sum;
  }, 0);

  const handleExportJSON = () => {
    const blob = new Blob([exportQuizAsJSON(quiz)], { type: 'application/json' });
  ...
  const handleExportAnki = () => {
    const blob = new Blob([exportQuizAsAnkiTSV(quiz)], { type: 'text/tab-separated-values' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
  ...
  const getScoreMessage = () => {
    ...
    if (score >= 50) return { message: 'Good effort!', color: 'text-yellow-500' };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="app-frame p-8 text-center">
        ...
        <div className="grid grid-cols-3 gap-4 max-w-md mx-auto mt-6">
          <div className="p-4 rounded-xl bg-app-box">
            ...
            <p className="text-2xl font-bold text-foreground">{correctCount}/{quiz.questions.length}</p>
          ...
          <div className="p-4 rounded-xl bg-app-box">
            ...
            <p className="text-2xl font-bold text-foreground">{formatTime(session.elapsedTime)}</p>
      ...
      <div className="app-frame p-6">
        <h3 className="text-lg font-semibold text-foreground mb-4">Export Quiz</h3>
          ...
          return (
            <div
              ...
              className={`app-frame p-6 border-l-4 ${
                isUnanswered ? 'border-l-gray-500' :
                ...
                <div className="flex-1 space-y-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-muted">{question.points || 1} pts</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className={`p-4 rounded-xl ${
                      isUnanswered ? 'bg-gray-500/10' :
                      isCorrect ? 'bg-green-500/10' : 'bg-red-500/10'
                    }`}>
                      <div className="flex items-center gap-2 mb-2">
                        {isUnanswered ? (
                          <span className="text-xs text-gray-500 uppercase tracking-wider">Not Answered</span>
                        ) : isCorrect ? (
                          <>
                            <Check className="w-4 h-4 text-green-500" />
                            <span className="text-xs text-green-500 uppercase tracking-wider">Your Answer</span>
                          </>
                        ) : (
                          <>
                            <X className="w-4 h-4 text-red-500" />
                            <span className="text-xs text-red-500 uppercase tracking-wider">Your Answer</span>
                          </>
                        )}
                      </div>
                      <p className={`font-medium ${
                        isUnanswered ? 'text-gray-400' :
                        isCorrect ? 'text-green-400' : 'text-red-400'
                      }`}>
                        {isUnanswered ? '—' : Array.isArray(userAnswer) ? userAnswer.join(', ') : userAnswer}
                      </p>
                    </div>
                    ...
                    {!isCorrect && (
                      <div className="p-4 rounded-xl bg-green-500/10">
                        <div className="flex items-center gap-2 mb-2">
                          <Check className="w-4 h-4 text-green-500" />
                          <span className="text-xs text-green-500 uppercase tracking-wider">Correct Answer</span>
                        </div>
